...
order_radius = [15, 25, 35, 40, 60, 75, 90, 110, 125]

# 확통 내용?


skwed_probabitity = [0.7, 0.3, 0, 0, 0, 0, 0, 0, 0]
score = 0

# pygame init
running = True
game_over = False
COOLDOWN_DURATION = 0  # 0초 1000 = 1초
...
class Ball():
    ...
    def __init__(self, pos=(0, 0), radius=order_radius[0], is_static=False):
        if is_static:
            ...
            self.body = pm.Body(100000000, 1)
            self.body.velocity_func = custom_gravity
    ...
    def ball_and_wall_collision(arbiter, space, _):  # 공과 벽 충돌 함수
        global game_over
        game_over = True
    # 공과 벽과의 충돌에 대한 충돌 핸들러 추가
    ball_wall_handler = space.add_collision_handler(1, 2)
    ball_wall_handler.begin = ball_and_wall_collision
...
PLANET = Ball((SCREEN_SIZE[0] / 2, SCREEN_SIZE[1] / 2), 60, is_static=True)
PLANET.set_visible()


def custom_gravity(body: pymunk.Body, gravity, damping, dt):
    planet_coord = PLANET.get_position()
    current_coord = body.position
    target_vec = ((planet_coord[0] - current_coord[0])*3,
                  (planet_coord[1] - current_coord[1])*3)
...
while running:
    ...
    pg.draw.circle(screen, PLANET.shape.color,
                   PLANET.get_position(), int(PLANET.shape.radius))
    if game_over:
        ...
        if event.type == pg.QUIT:
            ...
            if not cooldown_active:
                if event.pos[0] > 40 and event.pos[0] < SCREEN_SIZE[0] - 40:
                    # Draw a ball when the mouse is clicked
                    x = event.pos[0]
                    ...
                    # random setting
                    if score >= 4 and score < 12:
                        skwed_probabitity = [0.55, 0.4, 0.05, 0, 0, 0, 0, 0, 0]
                    elif score >= 12 and score < 32:
                        skwed_probabitity = [
                            0.35, 0.35, 0.25, 0.05, 0, 0, 0, 0, 0]
                    elif score >= 32 and score < 128:
                        skwed_probabitity = [0.3, 0.35,
                                             0.2, 0.1, 0.05, 0, 0, 0, 0]
                    elif score >= 128 and score < 272:
                        skwed_probabitity = [0.15, 0.35,
                    ...
                    preview_ball.set_visible()
            if current_time - last_ball_creation_time >= COOLDOWN_DURATION:
                cooldown_active = False

    # check current position is valid
    for ball in balls:
        pos = ball.get_position()
        pg.draw.circle(screen, ball.shape.color, pos, (ball.shape.radius))

        space.debug_draw(draw_options)
        # 벽 충돌 시 종료 함수
        if ball.shape.shapes_collide(wall_left_shape).points or \
           ball.shape.shapes_collide(wall_right_shape).points or \
           ball.shape.shapes_collide(wall_bottom_shape).points:
            game_over = True
            print(game_over)
    ...
    for i in range(len(balls)):
        ...
        for j in range(i+1, len(balls)):
            ...
            if is_hit and balls[i].shape.radius == balls[j].shape.radius:
                ...
                score += order_score[next_level_index - 1]
                # if level != max
